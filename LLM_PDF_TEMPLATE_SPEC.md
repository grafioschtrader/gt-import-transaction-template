...
| `tc3` | Double | Third transaction cost (e.g., an additional exchange execution fee). |
...
### Example 4: Three Separate Fees — CornerTrader (German)

The following excerpt belongs to a buy/sell template. Corneronline prints
three separate fees, followed or preceded by stamp duty:

```text
Handelsgebühr {tc1|SL|P|Nc|O}- CHF
Fremdgebühren {tc2|SL|P|Nc|O}- CHF
Stempel {tt1|SL|P|Nc|O}- CHF
Ausführungsgeb. Swx {tc3|SL|P|Nc|O}- CHF
```

Use `overRuleSeparators=All<.|,>` for this layout. `Nc` matches the trailing
minus sign; `O` allows a fee or tax line to be absent. The three fee amounts
are combined as `abs(tc1) + abs(tc2) + abs(tc3) - abs(reduce)`, with missing
values treated as zero. For fees of 4,85, 1,15 and 0,77 CHF, the transaction
cost is 6,77 CHF. Stamp duty of 11,53 CHF remains a separate tax (`tt1`).
Do not put the third fee in `tt2` or hide it using a rounding tolerance.

`tc3` requires a GT backend that supports the third fee field. Templates
without it continue to work unchanged. The shared field is available to
both PDF and CSV templates; CSV uses a column assignment such as
`tc3=Execution fee`.
...
- [ ] **Optional fields** (`tc1`, `tc2`, `tc3`, `tt1`, `tt2`, `ac`, `reduce`,
...
### Convert foreign-currency dividends to the cash account currency

For dividends, extract `cin` from the payout currency printed beside the distribution per unit, `cac` from the credited account, and `cex` as the multiplier from `cin` to `cac`. Preserve the document's original `quotation` and taxes at the parsing stage. Do not replace the payout currency with `cac` without converting the amounts.

GT then resolves the security using **ISIN + cac**, independently of holdings. With a valid explicit exchange rate, it converts quotation, taxes and fees into `cac`, retaining costs already in that currency. The resulting transaction is entirely in `cac`: GT sets its instrument currency to `cac` and clears the exchange rate to prevent a second conversion. The original payout amounts remain in the source document. The rounded cash credit must still match the document. A missing target security or rate, third cost currencies and inconsistent credits are left for review.

Test both extraction and import preparation: for example, ISIN DK0062498333 with `cac=EUR`, `cin=DKK`, 500 units, a DKK 3.75 distribution, DKK 506.25 tax and `cex=0.13378437` must yield the EUR security, EUR 0.50169139 per unit, EUR 67.73 tax and an unchanged EUR 183.12 cash credit, without a remaining exchange rate or a holdings query. This conversion requires a backend containing the dividend cash-currency correction.
